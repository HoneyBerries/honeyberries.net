...
/**
 * Gem card component for Minecraft server page
 * Displays elemental gem with collapsible details
 */
const GemCard = memo(function GemCard({ gem }) {
  const { name, element, passive, active, image, color } = gem;
  const [open, setOpen] = useState(false);

  // Color schemes for different gem elements
  const colorSchemes = {
    air: {
    ...
    fire: {
      bg: 'from-red-100 via-orange-50 to-amber-100',
      border: 'border-red-200',
      iconBg: 'bg-gradient-to-br from-red-400 via-orange-400 to-amber-500',
    ...
    ice: {
      bg: 'from-cyan-100 via-sky-50 to-indigo-100',
    ...
    darkness: {
      bg: 'from-slate-100 via-gray-50 to-violet-100',
    ...
    >
      <div className="flex items-start gap-4 flex-1">
        ...
        <div className="min-w-0 flex-1 flex flex-col">
          ...
          {/* Passive description */}
          <p className={`text-sm text-gray-700 leading-relaxed mb-3 ${open ? '' : 'line-clamp-1'}`}>
            {passive}
          </p>
          
          {/* Expandable active ability */}
